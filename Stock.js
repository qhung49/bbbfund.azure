...
`;
  ...
  static buyAsync(data) {
    ...
    var cashDecrease = data.numberShares * data.price * 1.035; // 3.5% transaction fee
    return this.getAllAsync()
      ...
      .then(function (portfolio) {
        ...
        var fee = data.numberShares * data.price * 0.035;
...
UPDATE Portfolio
SET number_shares = number_shares - ${cashDecrease * 100} 
...
`;
  ...
  static sellAsync(data) {
    ...
    return this.getAllAsync()
      ...
      .then(function () {
        var details = JSON.stringify(data);
        var cashIncrease = data.numberShares * data.price * 0.955; // 3.5% transaction fee, 1% tax
        var fee = data.numberShares * data.price * 0.045;
        var query = `
BEGIN TRAN T1
...
UPDATE Portfolio
SET number_shares = number_shares + ${cashIncrease * 100} 
WHERE stock_name = 'CASH'
...
UPDATE Portfolio
SET number_shares = number_shares + ${data.dividend * 100} 
WHERE stock_name = 'CASH'
...
COMMIT TRAN T1
  ...
  static validateStockExists(stockName) {
    ...
    return allStocks.find(s => s.name === stockName);
  }
}